...
from deep_research_from_scratch.utils import init_local_chat_model, get_structured_output_model

# --- 1. SETUP MODEL ---
# Ensure you have your API key set in env: GOOGLE_API_KEY
model = init_local_chat_model()
...
# Note: Structured output models are created lazily in node functions
# to support fallback to Gemini if local model doesn't support structured output
...
def clarify_with_user(state: State) -> Command[Literal["write_research_brief", "__end__"]]:
    ...
    # Set up structured output model (with fallback to Gemini if local model doesn't support it)
    structured_output_model = get_structured_output_model(ClarifyWithUser)
...
def write_research_brief(state: State):
    ...
    # Set up structured output model (with fallback to Gemini if local model doesn't support it)
    structured_output_model = get_structured_output_model(ResearchQuestion)
...
def generate_structure(state: State):
    ...
    # Get structured output model (with fallback to Gemini if local model doesn't support it)
